...
# Log File Date / Time Stamp:
$logtime = get-date -format "dd-MM-yyyy_hh-mm-ss"
# Log File Name:
$logfile = "Log_"+$logfile+".log"
...
function Export-LocalFiles ($blobs) {
    ...
    for ($i = 0; $i -le $blobs.Length -1; $i++) 
        { 
            ...
            $timeLeft = (($oauth.expires_on - (Get-Date -UFormat "%s"))) 
        }
...
function Export-MySQL ($blobs) {
    ...
    foreach ($record in $EXOmegaBlob)
    {
        #Construct the query to a valid string, then execute that sucker
        #Need to handle the parameters object by converting to a string.
        if ($record.Operation -eq "MailboxLogin") {
...
function Export-AzureSQL ($blobs) {
    ...
    Invoke-AzureSql -Query "if not exists (select * from sysobjects where name='auditexchange' and xtype='U') CREATE TABLE auditexchange (CreationTime DATETIME, Id CHAR(36) NOT NULL, Operation VARCHAR(100), OrganizationId VARCHAR(100), RecordType INT, ResultStatus VARCHAR(20), UserKey VARCHAR(100), UserType INT, Workload VARCHAR(50), ObjectId VARCHAR(256), UserId VARCHAR(100), ExternalAccess VARCHAR(20), OrganizationName VARCHAR(50), OriginatingServer VARCHAR(50), Parameters VARCHAR(100), ClientIPAddress VARCHAR(50), ClientInfoString VARCHAR(512), LogonType INT, LogonUserSid VARCHAR(100), MailboxGuid VARCHAR(100), MailboxOwnerSid VARCHAR(100), MailboxOwnerUPN VARCHAR(100), PRIMARY KEY (Id))"
    #Invoke-AzureSql -Query "if not exists (select * from sysobjects where name='auditaad' and xtype='U') CREATE TABLE auditaad (CreationTime DATETIME, Id CHAR(36) NOT NULL, Operation VARCHAR(100), OrganizationId VARCHAR(100), RecordType INT, ResultStatus VARCHAR(20), UserKey VARCHAR(100), UserType INT, Workload TEXT, ClientIP TEXT, ObjectId TEXT, UserId TEXT, AzureActiveDirectoryEventType TEXT, Client TEXT, LoginStatus TEXT, UserDomain TEXT, PRIMARY KEY (Id))"
    ...
    { 
        ...
        $timeLeft = (($oauth.expires_on - (Get-Date -UFormat "%s")))
        Write-Host "Reading blob #: " -NoNewLine; Write-Host ($i + 1) -ForegroundColor Green -NoNewline; Write-Host " out of " -NoNewline; Write-Host $blobs.Length -ForegroundColor Yellow -NoNewline; Write-Host " to memory. Current token lifespan is" $timeLeft
    ...
    foreach ($record in $EXOmegaBlob)
    {
        ...
        #Need to handle the parameters object by converting to a string.
        if ($record.Operation -eq "MailboxLogin") {
        $thisQuery = "if not exists (select Id from auditexchange where Id='" + $record.Id + "') BEGIN INSERT INTO auditexchange (CreationTime, Id, Operation, OrganizationId, RecordType, ResultStatus, UserKey, UserType, Workload, ObjectId, UserId, ExternalAccess, OrganizationName, OriginatingServer, Parameters, ClientIPAddress, ClientInfoString, LogonType, LogonUserSid, MailboxGuid, MailboxOwnerSid, MailboxOwnerUPN) VALUES ('" + $record.CreationTime + "', '" + $record.Id + "', '" + $record.Operation + "', '" + $record.OrganizationId + "', '" + $record.RecordType + "', '" + $record.ResultStatus + "', '" + $record.UserKey + "', '" + $record.UserType + "', '" + $record.Workload + "', '" + $record.ObjectId + "', '" + $record.UserId + "', '" + $record.ExternalAccess + "', '" + $record.OrganizationName + "', '" + $record.OriginatingServer + "', '" + $record.Parameters + "', '" + $record.ClientIPAddress + "', '" + $record.ClientInfoString + "', '" + $record.LogonType + "', '" + $record.LogonUserSid + "', '" + $record.MailboxGuid + "', '" + $record.MailboxOwnerSid + "', '" + $record.MailboxOwnerUPN + "') END"
        #Write-host $thisQuery
        Invoke-AzureSql "$thisQuery"
        }
    }
    Write-Host "#####################################################"
